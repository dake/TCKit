...
static CGBitmapInfo const kTCSystemDefaultOrder = kCGBitmapByteOrderDefault | kCGImageAlphaPremultipliedLast;

@implementation UIImage (CGImage)

+ (CGBitmapInfo)defaultBitMapOrder
{
    return kTCDefaultBitMapOrder;
}

+ (CGBitmapInfo)systemDefaultBitMapOrder
{
    return kTCSystemDefaultOrder;
}

+ (BOOL)isDefaultBitMapOrder:(CGBitmapInfo)info
{
    return ((info & kCGBitmapAlphaInfoMask) | (info & kCGBitmapByteOrderMask)) == kTCDefaultBitMapOrder;
}

+ (BOOL)isSystemDefaultBitMapOrder:(CGBitmapInfo)info
{
    return ((info & kCGBitmapAlphaInfoMask) | (info & kCGBitmapByteOrderMask)) == kTCSystemDefaultOrder;
}
...
+ (NSData *)dataFromCGImage:(CGImageRef)cgImage info:(TCImageInfo *)info redraw:(BOOL)forceRedraw
{
    ...
    BOOL shouldRedrawGraphic = (bitmapInfo & kTCDefaultBitMapOrder) != kTCDefaultBitMapOrder ||
    realWidth != width ||
    channel != 4 ||
    bitsPerComponent != 8;
